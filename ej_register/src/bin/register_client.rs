...
fn main() -> std::io::Result<()> {
    if env::args().len() != NUMBER_OF_ARGS {// control de que me pasen los argumentos necesarios
        panic!("cargo run --bin register-client ip-servidor puerto-servidor");
    }
    ...
    println!("Conectado al servidor de registros. Introduce comandos (\".q\" para salir):");
    ...
    for line_result in std::io::stdin().lines() { // para cada linea del buffer
        ...
        if let Some(&command) = words.get(0) { // si me devuelve un Ok()
            match command { //matcheo lo que quiero hacer depende del comando
                ...
                ".consult" => { //consultar si hay un usuario registrado
                    let user = words[1]; // me quedo con la segunda palabra del vec (el nombre del usuario)
                    writeln!(stream, ".consult {}", user)?; // le mando el nombre de usuario del cual quiero saber si esta registrado
                }
                ".register" => { // registrar usuario + constraseña
                    writeln!(stream, "{}", line.trim())?;// devolver la linea entera, que ya tiene comando + palabra + significado
                }
